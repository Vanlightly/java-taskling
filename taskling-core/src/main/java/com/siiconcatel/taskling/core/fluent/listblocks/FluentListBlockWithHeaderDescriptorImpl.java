...
public class FluentListBlockWithHeaderDescriptorImpl<T,H>  implements FluentListBlockWithHeaderDescriptor<T,H> {

    private ObjectMapper mapper;

    public FluentListBlockWithHeaderDescriptorImpl() {
        this.mapper = new ObjectMapper();
    }

    @Override
    public OverrideConfigurationDescriptor withSingleUnitCommit(List<T> values, H header, short maxBlockSize)
    {
        List<String> jsonValues = serialize(values);
        String jsonHeader = serialize(header);
        FluentBlockSettingsDescriptorImpl listBlockDescriptor = new FluentBlockSettingsDescriptorImpl(jsonValues, jsonHeader, maxBlockSize);
    ...
    @Override
    public OverrideConfigurationDescriptor withPeriodicCommit(List<T> values, H header, short maxBlockSize, BatchSize batchSize)
    {
        List<String> jsonValues = serialize(values);
        String jsonHeader = serialize(header);
        FluentBlockSettingsDescriptorImpl listBlockDescriptor = new FluentBlockSettingsDescriptorImpl(jsonValues, jsonHeader, maxBlockSize);
    ...
    @Override
    public OverrideConfigurationDescriptor withBatchCommitAtEnd(List<T> values, H header, short maxBlockSize)
    {
        List<String> jsonValues = serialize(values);
        String jsonHeader = serialize(header);
        FluentBlockSettingsDescriptorImpl listBlockDescriptor = new FluentBlockSettingsDescriptorImpl(jsonValues, jsonHeader, maxBlockSize);
    ...
    private String serialize(H value)
    {
        return TasklingSerde.serialize(value, false);
    }
}
